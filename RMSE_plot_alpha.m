...
m =2;
alp =200:200:800;
Rmse_2=zeros(length(alp),L);
Rmse_3=zeros(length(alp),L);
...
RMSE_MAP=(1/T)*sum(sqrt((1/l)*sum(sum(Position_diff_MAP.^2+Velocity_diff_MAP.^2),3)));
RMSE_OEKF = (1/T)*sum(sqrt((1/l)*sum(sum(Position_diff_OEKF.^2+Velocity_diff_OEKF.^2),3)));

Rmse_2(ind,l) = RMSE_MAP;
Rmse_3(ind,l) = RMSE_OEKF;
end
...
data = {Rmse_2',Rmse_3'};
boxplotGroup(data,'PrimaryLabels',{'MAP','MOD'},'SecondaryLabels',{'alpha = 200' 'alpha = 400' 'alpha = 600' 'alpha = 800'},'GroupLabelType','Vertical')
