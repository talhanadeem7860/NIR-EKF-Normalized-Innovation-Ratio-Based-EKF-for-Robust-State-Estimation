...
Trmse_2 = zeros(length(lam),L);
Trmse_3 = zeros(length(lam),L);
% Trmse_1 = zeros(7,4);
% Trmse_2 = zeros(7,4);
% Trmse_3 = zeros(7,4);
% Trmse_1=[];
...
T_OEKF = [];
alphaa = [200];
for a = 1:length(alphaa)
    alpha = alphaa(a);
for ind = 1:length(lam)
    lambda = lam(ind);
...
% 
% Trmse_2(ind,l) = TRMSE_MAP;
% Trmse_3(ind,l) = TRMSE_OEKF;
end
...
% boxplotGroup(data,'PrimaryLabels',{'MAP','MOD'},'SecondaryLabels',{'lambda = 0' 'lambda = 0.3' 'lambda = 0.5' 'lambda = 0.6'},'GroupLabelType','Vertical')
% title('m = 30, L = 100, alpha = 200')
% 


% plot(lam,Trmse_1,'r');
% hold on;
% plot(lam,Trmse_2(:,1),'go-', linewidth=2, markersize=6);
% hold on
% plot(lam,Trmse_3(:,1),'g*-', linewidth=2, markersize=6);
% 
% plot(lam,Trmse_2(:,2),'bo-', linewidth=2, markersize=6);
% plot(lam,Trmse_3(:,2),'b*-', linewidth=2, markersize=6);
% 
% plot(lam,Trmse_2(:,3),'ro-', linewidth=2, markersize=6);
% plot(lam,Trmse_3(:,3),'r*-', linewidth=2, markersize=6);
% 
% plot(lam,Trmse_2(:,4),'yo-', linewidth=2, markersize=6);
% plot(lam,Trmse_3(:,4),'y*-', linewidth=2, markersize=6);
% 
% 
% 
% ylabel('RMSE Vector');
% xlabel('lambda');
% legend('alpha_{MAP} = 50','alpha_{MOD} = 50','alpha_{MAP} = 200','alpha_{MOD} = 200','alpha_{MAP} = 750','alpha_{MOD} = 750','alpha_{MAP} = 1000','alpha_{MOD} = 1000')
